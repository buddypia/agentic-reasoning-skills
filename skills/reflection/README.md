# Multi-LLM Reflection — 설치·사용 가이드

서로 다른 벤더의 LLM이 생성→비판→개선 3단계로 고품질 결과를 만드는 워크플로우.
스킬 정의(호출 요약) → [SKILL.md](./SKILL.md)

## 동작 방식

```
[작업] → Generator(생성) → Critic(비판) → Refiner(개선) → 최종본
          agy/Gemini3.5     claude/opus-4-8  codex/gpt-5.5(xhigh)
```
각 단계는 독립 컨텍스트에서 구조화 JSON을 출력하며, 비판 결과를 개선 단계가 반영한다.
...
| CLI | 단계 | 설치 | 인증 |
|-----|------|------|------|
| `agy` (Antigravity CLI) | 생성 | https://antigravity.google → 설치 후 `agy install` | `agy` 최초 실행 시 OAuth |
| `claude` (Claude Code) | 비판 | `npm i -g @anthropic-ai/claude-code` | `claude` 실행 → 로그인(구독) |
| `codex` (Codex CLI) | 개선 | `npm i -g @openai/codex` | `codex login` (ChatGPT 구독) |
...
### run.sh (권장)
```bash
<skill-dir>/scripts/run.sh "신규 SaaS의 기술 아키텍처를 설계해줘

[컨텍스트]
- B2B 청구관리 / 초년 1000사 / AWS·SOC2 준수 / 인프라 월 50만엔 이내"

<skill-dir>/scripts/run.sh --verbose "..."   # 3단계 상세 출력
...
python scripts/main.py "작업" \
    --generator-model gemini-3.5-flash \
    --critic-model    claude-opus-4-8 \
    --refiner-model   gpt-5.5
# 프로바이더 교체: --generator-provider {gemini|anthropic|openai|mock}
```
...
| `REFLECTION_{GENERATOR,CRITIC,REFINER}_{PROVIDER,MODEL}` | — | 역할별 오버라이드 |
